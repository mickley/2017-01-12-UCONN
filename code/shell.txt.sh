...
:ESCOA	[file ...]
:ESCOls [-ABCFGHLOPRSTUW@abcdefghiklmnopqrstuwx1]
SYNOPSIS
:ESCOA
:ESCOls -- list directory contents
NAMEOA
:ESCOA
:ESCOA	LS(1)
:ESCOA			  BSD General Commands Manual
:ESCOnames of files contained within that directory, as
:ESCOwell as any requested, associated information.
:ESCOB
:ESCOIf no operands are given, the contents of the current
...
:ESCOB
:ESCO-f      Output is not sorted.  This option turns on
:ESCOB	     the -a option.
:ESCOB
:ESCO-G      Enable colorized output.  This option is
:ESCOB	     equivalent to defining CLICOLOR in the envi-
:ESCOB	     ronment.  (See below.)
:ESCOB
:ESCO-g      This option is only available for compatibil-
:ESCOB	     ity with POSIX; it is used to display the
:ESCOB	     group name in the long (-l) format output
:ESCOB	     (the owner name is suppressed).
:ESCOB
:ESCO-H      Symbolic links on the command line are fol-
:ESCOB	     lowed.  This option is assumed if none of the
:ESCOB	     -F, -d, or -l options are specified.
:ESCOB
:ESCO-h      When used with the -l option, use unit suf-
:ESCOB	     fixes: Byte, Kilobyte, Megabyte, Gigabyte,
:ESCOB	     Terabyte and Petabyte in order to reduce the
:ESCOB	     number of digits to three or less using base
:ESCOB	     2 for sizes.
:ESCOB
:ESCO-i      For each file, print the file's file serial
:ESCOB	     number (inode number).
:ESCOB
:ESCO-k      If the -s option is specified, print the file
:ESCOB	     size allocation in kilobytes, not blocks.
:ESCOB	     This option overrides the environment vari-
:ESCOB	     able BLOCKSIZE.
:ESCOB
:ESCO-L      Follow all symbolic links to final target and
:ESCOB	     list the file or directory the link refer-
:ESCOB	     ences rather than the link itself.  This
:ESCOB	     option cancels the -P option.
$ clear
$ ls -F Desktop
